...
class TestDeployAgent:
    async def test_deploy_agent_with_ssh_session(self, msf, empire_client_mock):
        ...
        ret = await empire.deploy_agent({"session_id": "1", co.STAGER_ARGUMENTS: {"agent_name": "agent1"}})
        assert ret == {co.RETURN_CODE: 0, 'std_out': "Agent 'agent1' deployed on target test_target."}
    ...
    async def test_deploy_agent_session_id_not_found(self, msf, empire_client_mock):
        ...
        ret = await empire.deploy_agent({"session_id": "1", co.STAGER_ARGUMENTS: {"agent_name": "agent1"}})
        assert ret == {co.STD_ERR: "Msf Session with id 'test_id' not found.", co.RETURN_CODE: -2}
    ...
    async def test_deploy_agent_with_ssh_connection(self, mocker, empire_client_mock):
        ...
        ret = await empire.deploy_agent({"ssh_connection": {"target": "test_target"},
                                         co.STAGER_ARGUMENTS: {"agent_name": "agent1"}})
    ...
    async def test_deploy_agent_paramiko_error(self, mocker, empire_client_mock):
        ...
        ret = await empire.deploy_agent({"ssh_connection": {"target": "test_target"},
                                         co.STAGER_ARGUMENTS: {"agent_name": "agent1"}})
        assert ret == {co.RETURN_CODE: -2, 'std_err': "fake_error"}
    ...
    async def test_deploy_agent_with_ssh_connection_key_error(self, mocker, empire_client_mock):
        ...
        ret = await empire.deploy_agent({"ssh_connection": {}, co.STAGER_ARGUMENTS: {}})
        assert ret == {co.STD_ERR: f"Missing 'target' argument in ssh_connection.", co.RETURN_CODE: -2}
    ...
    async def test_deploy_agent_payload_key_error(self, mocker, empire_client_mock):
        ...
        ret = await empire.deploy_agent({"session_id": "1", co.STAGER_ARGUMENTS: {"agent_name": "agent1"}})
        assert ret == {co.STD_ERR: "", co.RETURN_CODE: -2}
...
class TestEmpireClient:
    ...
    async def test_execute_on_agent(self, empire_client):
        module_arguments = {"use_agent": "test_agent", "empire_module": "test_module"}
    ...
    async def test_execute_on_agent_agent_not_found(self, empire_client):
        ...
        ret = await empire_client.execute_on_agent({"use_agent": "test_agent", "empire_module": "test_module"})
        assert ret == {"return_code": -2, "std_err": "Agent 'test_agent' not found in Empire."}
    
    async def test_execute_on_agent_module_not_found(self, empire_client):
        empire_client.modules.get.side_effect = KeyError()
        ret = await empire_client.execute_on_agent({"use_agent": "test_agent", "empire_module": "test_module"})
        assert ret == {"return_code": -2, "std_err": "Module 'test_module' not found in Empire."} 
    ...
    async def test_execute_on_agent_module_execution_error(self, mocker, empire_client):
        ...
        ret = await empire_client.execute_on_agent({"use_agent": "test_agent", "empire_module": "test_module"})
        assert ret == {"return_code": -2, "std_err": "module_error"}
